...
| **BUG-117** | Cannot read properties of null (reading 'useRef') | `general` | `critical` | `auto-crash-handler` | `claude-cli` | Same failure class as BUG-115 -- a null useRef inside TripMapHero pointing at a broken React instance is the downstream symptom of a dynamic-import chunk load left half-resolved. Fixed in commit 8b47dae via lazyImport(), which reloads once on that failure instead of leaving the module graph partially loaded. |
| **BUG-118** | Transition was skipped | `general` | `critical` | `auto-crash-handler` | `claude-cli` | Fixed in commit 8b47dae: withViewTransition() now attaches .catch() to the transition's ready/finished promises. This was an unhandled rejection, not a crash -- the browser rejects those promises whenever a newer view transition interrupts an older one (rapid tab/trip switching), which is expected behavior, not a failure. |
| **BUG-119** | Transition was skipped | `general` | `critical` | `auto-crash-handler` | `claude-cli` | Fixed in commit 8b47dae: withViewTransition() now attaches .catch() to the transition's ready/finished promises. This was an unhandled rejection, not a crash -- the browser rejects those promises whenever a newer view transition interrupts an older one (rapid tab/trip switching), which is expected behavior, not a failure. |
| **BUG-120** | Transition was skipped | `general` | `critical` | `auto-crash-handler` | `claude-cli` | Fixed in commit 8b47dae: withViewTransition() now attaches .catch() to the transition's ready/finished promises. This was an unhandled rejection, not a crash -- the browser rejects those promises whenever a newer view transition interrupts an older one (rapid tab/trip switching), which is expected behavior, not a failure. |
...
| **BUG-123** | Old view transition aborted by new view transition. | `general` | `critical` | `auto-crash-handler` | `claude-cli` | Fixed in commit 8b47dae: withViewTransition() now attaches .catch() to the transition's ready/finished promises. This was an unhandled rejection, not a crash -- the browser rejects those promises whenever a newer view transition interrupts an older one (rapid tab/trip switching), which is expected behavior, not a failure. |
| **BUG-124** | Transition was skipped | `general` | `critical` | `auto-crash-handler` | `claude-cli` | Fixed in commit 8b47dae: withViewTransition() now attaches .catch() to the transition's ready/finished promises. This was an unhandled rejection, not a crash -- the browser rejects those promises whenever a newer view transition interrupts an older one (rapid tab/trip switching), which is expected behavior, not a failure. |
| **BUG-125** | Transition was skipped | `general` | `critical` | `auto-crash-handler` | `claude-cli` | Fixed in commit 8b47dae: withViewTransition() now attaches .catch() to the transition's ready/finished promises. This was an unhandled rejection, not a crash -- the browser rejects those promises whenever a newer view transition interrupts an older one (rapid tab/trip switching), which is expected behavior, not a failure. |
| **BUG-126** | Transition was aborted because of invalid state | `general` | `critical` | `auto-crash-handler` | `claude-cli` | Fixed in commit 8b47dae: withViewTransition() now attaches .catch() to the transition's ready/finished promises. This was an unhandled rejection, not a crash -- the browser rejects those promises whenever a newer view transition interrupts an older one (rapid tab/trip switching), which is expected behavior, not a failure. |
| **BUG-127** | Transition was skipped | `general` | `critical` | `auto-crash-handler` | `claude-cli` | Fixed in commit 8b47dae: withViewTransition() now attaches .catch() to the transition's ready/finished promises. This was an unhandled rejection, not a crash -- the browser rejects those promises whenever a newer view transition interrupts an older one (rapid tab/trip switching), which is expected behavior, not a failure. |
| **BUG-128** | Transition was skipped | `general` | `critical` | `auto-crash-handler` | `claude-cli` | Fixed in commit 8b47dae: withViewTransition() now attaches .catch() to the transition's ready/finished promises. This was an unhandled rejection, not a crash -- the browser rejects those promises whenever a newer view transition interrupts an older one (rapid tab/trip switching), which is expected behavior, not a failure. |
| **BUG-129** | Transition was skipped | `general` | `critical` | `auto-crash-handler` | `claude-cli` | Fixed in commit 8b47dae: withViewTransition() now attaches .catch() to the transition's ready/finished promises. This was an unhandled rejection, not a crash -- the browser rejects those promises whenever a newer view transition interrupts an older one (rapid tab/trip switching), which is expected behavior, not a failure. |
| **BUG-130** | Transition was skipped | `general` | `critical` | `auto-crash-handler` | `claude-cli` | Fixed in commit 8b47dae: withViewTransition() now attaches .catch() to the transition's ready/finished promises. This was an unhandled rejection, not a crash -- the browser rejects those promises whenever a newer view transition interrupts an older one (rapid tab/trip switching), which is expected behavior, not a failure. |
| **BUG-131** | Transition was skipped | `general` | `critical` | `auto-crash-handler` | `claude-cli` | Fixed in commit 8b47dae: withViewTransition() now attaches .catch() to the transition's ready/finished promises. This was an unhandled rejection, not a crash -- the browser rejects those promises whenever a newer view transition interrupts an older one (rapid tab/trip switching), which is expected behavior, not a failure. |
| **BUG-132** | An unknown Component is an async Client Component. Only Server Components can be async at the moment. This error is often caused by accident | `general` | `critical` | `auto-crash-handler` | `claude-cli` | Same failure class as BUG-115/117 -- React misreading a stalled/half-resolved lazy import as an async component is a downstream symptom of the same chunk-load race. Fixed in commit 8b47dae via lazyImport(). |
| **BUG-133** | An unknown Component is an async Client Component. Only Server Components can be async at the moment. This error is often caused by accident | `general` | `critical` | `auto-crash-handler` | `claude-cli` | Duplicate of BUG-132 (same message, same root cause). Fixed in commit 8b47dae via lazyImport(). |
| **BUG-134** | Transition was skipped | `general` | `critical` | `auto-crash-handler` | `claude-cli` | Fixed in commit 8b47dae: withViewTransition() now attaches .catch() to the transition's ready/finished promises. This was an unhandled rejection, not a crash -- the browser rejects those promises whenever a newer view transition interrupts an older one (rapid tab/trip switching), which is expected behavior, not a failure. |
...
| **BUG-159** | Missing autocomplete attributes on email/password inputs breaks autofill on mobile | `ui-ux` | `medium` | `claude-cli` | `claude-cli` | Added autoComplete attributes to all email/password inputs across LoginScreen, ResetPasswordScreen, SuperadminAuthModal, AdminToolsPage |
