...
- **Rule -1 (Explicit User Command Mandatory Before Push - Zero Autonomous Pushes):** You MUST NEVER push code to GitHub or any remote repository unless the user explicitly and directly commands you to do so (e.g. "commit and push", "push to github", "git push"). Even if a feature is complete, builds pass, and tests succeed, ALL changes must remain strictly local until the user explicitly instructs you to commit and push. Never proactively, automatically, or preemptively commit or push to any repository.
